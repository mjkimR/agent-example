...
class AICatalogItem(BaseModel):
    ...
    kind: Literal["model", "alias"] = Field(
        ..., description="Indicates whether this is a model or alias"
    )
    type: AIModelType = Field(..., description="The type of the model")
    help: str | None = Field(
        default=None, description="Description of the model or alias"
    )
...
class AIModelParamSpec(BaseModel):
    exclude: list[str] = Field(
        default_factory=list, description="List of parameter names to exclude"
    )
    map: dict[str, str] = Field(
        default_factory=dict, description="Mapping of parameter names"
    )
...
class AIModelItem(BaseModel):
    ...
    args: dict[str, Any] = Field(
        default_factory=dict, description="Arguments for model initialization"
    )
    fallbacks: list[str] = Field(
        default_factory=list, description="List of fallback model/alias names"
    )
    kind: Literal["model"] = Field(
        default="model", description="Indicates this is a model item"
    )
...
class AIModelAliasItem(BaseModel):
    ...
    kind: Literal["alias"] = Field(
        default="alias", description="Indicates this is an alias item"
    )
...
class AIModelGroupItem(BaseModel):
    ...
    members: list[AICatalogItem] = Field(
        ..., description="List of models/aliases in this group"
    )
    ...
    def from_data(
        ...
        for _member_name in _member_names:
            if _member_name not in catalogs:
                raise ValueError(
                    f"Model group '{_name}' has unknown member '{_member_name}'."
                )
        ...
        if _default not in _member_names:
            raise ValueError(
                f"Model group '{_name}' has default '{_default}' which is not in its members list."
            )
